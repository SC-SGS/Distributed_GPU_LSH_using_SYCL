 ...
 *
 * @attention Before calling `END_TIMING_MPI(x, comm)` a call to `START_TIMING(x)` **must** be made!
 *
 *
 ...
 * @param[in] queue the SYCL queue to call `wait()` on
 *
 * @attention Before calling `END_TIMING_MPI_AND_BARRIER(x, comm, queue)` a call to `START_TIMING(x)` **must** be made!
 */
#ifdef ENABLE_TIMING
...
#define END_TIMING_MPI(name, communicator)                                                                                     \
do {                                                                                                                                \
const auto end_##name = std::chrono::steady_clock::now();                                                                           \
const auto duration_##name = std::chrono::duration_cast<std::chrono::milliseconds>(end_##name - start_##name).count();              \
int comm_rank_##name;                                                                                                               \
...
#define END_TIMING_MPI_AND_BARRIER(name, communicator, queue)                                                                  \
do {                                                                                                                                \
queue.wait();                                                                                                                       \
const auto end_##name = std::chrono::steady_clock::now();                                                                           \
const auto duration_##name = std::chrono::duration_cast<std::chrono::milliseconds>(end_##name - start_##name).count();              \
