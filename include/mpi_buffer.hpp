...
public:
     ...
     */
    mpi_buffers(const size_type size, const size_type dims, const MPI_Comm& communicator) :
            rank_size(size), dims(dims), communicator_(communicator),
            active_buffer_(0), buffer_0_(size * dims), buffer_1_(size * dims)
    {
    ...
    mpi_buffers(mpi_buffers&& other) noexcept
        : rank_size(other.rank_size), dims(other.dims), communicator_(other.communicator_),
          active_buffer_(other.active_buffer_), dest_(other.dest_), source_(other.source_),
          buffer_0_(std::move(other.buffer_0_)), buffer_1_(std::move(other.buffer_1_)) { }
