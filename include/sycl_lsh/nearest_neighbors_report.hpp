 ...
 * @param[in] logger the used mpi::logger
 ...
 */
[[nodiscard]] real_type recall(const aos_matrix<index_type> &calculated_indices, const aos_matrix<index_type> &correct_indices, const mpi::communicator &comm, const mpi::logger &logger);
 ...
 * @throws sycl_lsh::exception if the parsed number of dimensions doesn't match with the current `dims`.
 */
[[nodiscard]] std::tuple<real_type, index_type, index_type> error_ratio(const aos_matrix<real_type> &calculated_distances, const aos_matrix<real_type> &correct_distances, const mpi::communicator &comm, const mpi::logger &logger);
