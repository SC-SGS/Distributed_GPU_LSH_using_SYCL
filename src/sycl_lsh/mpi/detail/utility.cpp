...
void mpi_error_check(const int err) {
    if (err != MPI_SUCCESS) {
        if (const std::string err_msg = mpi_error_code_to_string(err); !err_msg.empty()) {
...
std::string mpi_error_code_to_string(const int error_code) {
    ...
    int err_str_len{};
    if (const int res = MPI_Error_string(error_code, err_str.data(), &err_str_len); res == MPI_SUCCESS) {
