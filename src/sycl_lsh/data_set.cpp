...
void data_set::init(const mpi::communicator &comm, const std::string &filename, const mpi::file_parser_type file_parser) {
    ...
    data_aos_ptr_ = std::make_shared<aos_matrix<real_type>>(parser->parse_content());
    // automatically converts the read data from AoS to SoA layout
    data_soa_ptr_ = std::make_shared<soa_matrix<real_type>>(*data_aos_ptr_);
