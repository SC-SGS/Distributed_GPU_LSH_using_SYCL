...
//                    local_mem[i + local_idx * data.dims] = acc_data[data.get_linear_id(idx, i + 100)]; // AoS
                }
        ...
        for (size_type hash_table = 0; hash_table < opt.num_hash_tables; ++hash_table) {
            for (size_type hash_function = 0; hash_function < opt.num_hash_functions; ++hash_function) {
                for (size_type dim = 0; dim < data.dims; ++dim) {
                    acc[this->get_linear_idx<memory_layout::aos>(hash_table, hash_function, dim, data, opt)] = std::abs(rnd_normal_dist(rnd_normal_gen));
                }
...
hash_tables(sycl::queue&, const Data&, const Options&)
    -> hash_tables<typename Options::real_type, typename Options::index_type, typename Options::hash_value_type>;
