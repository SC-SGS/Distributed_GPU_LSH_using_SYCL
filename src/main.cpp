...
int main() {
    ...
    constexpr std::size_t number_of_hashes = 32;
    // w
    constexpr real_t w = 1.0;
    constexpr index_t prim = 105613;
    constexpr std::size_t size = 2048 * 2048;
        ...
        queue.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<class hash_point>(execution_range, [=](sycl::nd_item<1> item_id) {
                ...
                for (std::size_t i = 0; i < dim; ++i) {
                    local_mem[i + local_idx * dim] = acc_d[i + idx * dim];
                }
                ...
                for (std::size_t j = 0; j < number_of_hashes; ++j) {
                    ...
                    for (std::size_t d = 0; d < dim; ++d) {
                        hash += local_mem[d + local_idx * dim] * acc_hf[d + j * (dim + 1)];
                    }
